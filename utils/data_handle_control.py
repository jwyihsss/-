...
class Config(BaseModel):
    ...
    cache_path: Union[str, Path] = 'test_data/cache.yml'
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                if isinstance(value, dict) or isinstance(value, list):
                    ...
                    # data[key] = data[key].replace('{{%s}}' % f'{func}', str(fake(func)))
                elif isinstance(value, str) and '$cache' in value:
                    cache_name = value[value.find('.') + 1:]
                    data[key] = self.load_cache().get(cache_name, None)
                elif isinstance(value, str) and 'int.' in value:
                    func = value[value.find('{') + 2:value.find('}')]
                    data[key] = int(data[key].replace('{{%s}}' % f'{func}', str(Execute(func[func.find('.') + 1:])())))
                elif isinstance(value, str) and 'str.' in value:
                    func = value[value.find('{') + 2:value.find('}')]
                    data[key] = str(data[key].replace('{{%s}}' % f'{func}', str(Execute(func[func.find('.') + 1:])())))
        elif isinstance(data, list):
            for item in data:
                if isinstance(item, dict) or isinstance(item, list):
                    self.replace_values(item)
                elif isinstance(item, str) and '{{' in item and '}}' in item and 'str.' not in item and 'int.' not in item:
                    ...
                    data[data.index(item)] = self.load_cache().get(cache_name, None)
                elif isinstance(item, str) and 'int.' in item:
                    func = item[item.find('{') + 2:item.find('}')]
                    data[data.index(item)] = int(
                        data[data.index(item)].replace('{{%s}}' % f'{func}', str(Execute(func[func.find('.') + 1:])())))
